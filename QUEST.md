...
- J105_박민서
  ...
  J062_김준기
  - 프로페셔널이란 주어진 일을 일정 안에 무조건 수행하는 것이 아니다.
  - 자신이 일정 안에 해결할 수 있는지 판단하고 그럴 수 없다면 관리자에게 이를 알려야 한다.
  - 관리자의 눈치를 보며 자신이 프로젝트를 일정 안에 해결할 수 있다고 하는 것은 회사에도 해를 끼치며, 본인의 일상에도 해를 끼친다.
  - 대신 상급자에게 안된다고 할 때는 그에 대한 대안을 말해야 한다.
  - 프로젝트를 수행하기 위해 밤을 새며 자신의 일상을 침범하는 것은 올바르지 않다. 
...
>최적화의 규칙
>규칙 1: 하지 마라
>규칙 2: 아직 하지 마라(전문가 전용)
>  - 마이클 잭슨
- 성능 최적화는 중요한 조건이지만, 일단 코드를 동작하게 만드는 것이 우선입니다. 대부분 코드는 아마 성능을 최적화할 필요가 없을 것입니다.
- 코드 최적화를 신경쓰지 말고, 일단 구현하세요. 그럼에도 불구하고 성능이 필요한 코드가 있다면, 먼저 프로파일링을 해야 합니다.

J166 유준상

- 일단 동작하게 만든 다음에 더 좋게 만들어라
    - 동작하지 않는 코드는 쓸모가 없다.
    - 코드에서 개선을 먼저 생각하지 말고 일단 요구사항을 구현하는게 중요하다고 말합니다.
    - 이번 챌린지 미션을 진행하면서 설계를 처음부터 완벽하게 하려고 너무 오랜시간을 사용해 구현을 끝까지 못마친 미션이 많았습니다.
    - 이때 먼저 요구사항을 구현하고 그 다음에 설계를 변경하는 방식을 지향해야 한다고 생각합니다.
- 코드를 많이 읽고 많이 써봐야지 잘 쓸 수 있다
    - 이번주 페어로 하는 활동에서 다른분의 코드를 많이 읽었었는데 힘들었지만 배우는 것도 많았습니다.
    - 솔직히 챌린지때 피어 컴파일링 시간에 남의 코드를 어렵고 시간이 많이 걸려서 많이 읽지 못했습니다.
    - 남의 코드를 분석하는 시간을 가져야겠다고 생각했습니다.
 
J255 정원철
- 짝 프로그래밍은 대화를 통해 추상화하고 하나의 키보드를 통해 구체화한다. 그리고 키보드를 짝끼리 교차하며 코드의 문제를 발견한다.
- 코드를 추상화하는 좋은 방법들이 많지만 다른 방법들을 모두 사용하면서도 추가적으로 추상화 할 수 있는 소프트스킬로써 짝 프로그래밍이 제시되었다. 
- 서로 대화하고 같이 고민하는 것으로 추상화를 심화하는 것은 큰 이점 같다.
- 이번 짝 프로그래밍을 하면서도 경험한 이 시나리오에 따라 설계와 코드의 추상성을 더 높일 수 있었다.
- 이전 알고 있던 혼자서는 놓치기 쉬운 문제를 짝이 찾아낸다는 장점에 더해 페어 프로그래밍을 통해 추상성을 높이는 과정, 이것은 좋은 경험이고 앞으로도 계속 시도하고 싶은 작업이다.

J155 양현호
- 기억이 오래가려면 연결고리가 있어야 한다고 합니다. JK님이 적으신 팁..? 경험..? 부분은 현재 챌린지, 베이직 과정에 적용이 되어있었고 읽으면서 나의 챌린지, 베이직 과정과 연결지어서 읽다보니 책의 내용이 더욱 기억에 잘 남았던거 같습니다.
- 앞으로의 학습도 현재의 기억이나 경험과 연결고리를 만들어 기억하면 좋겠다는 생각이 들었습니다.
...
- 실수를 아예 발생시키지 않으면 나중에 더 큰 실수가 되어 사고를 낼 수 있음. 모든 실수를 피해가려고 하기보단 실수가 발생하더라도 즉각적으로 대처한다면 더욱 도움이 될 것.
- 적극적으로 배우기
    - 배우는 과정에 내가 이것을 어디에 활용할 수 있을까 생각해보기
    - 무언가 현재까지 배운 내용을 갖고 만들어낼 수 있다면, 일단 만들어보기. 다시 막혔을 때 돌아오기.
...
---
**[ 다음주 챌린지 수료식에 정장 입고 오기 ](2024/08/02 추가)**

✔️ 의도 : 4주간 달려온 우리 모두를 멋지게 축하하자.

✔️ 수행기준 : 셔츠, 타이(필수), 재킷(선택가능), 와인잔 지참

---
**(**2024/08/02 **삭제)[ 미션을 수행하면서 만난 오류에 대한 원인의 사례 알아보기 ] **
